  ...
  switch (lhs.type_) {
    ...
    case type_id::DOUBLE:
      switch (op) {
        ...
        case CONDITIONAL_OR:
          if (lhs.type_ == type_id::DOUBLE || rhs.type_ == type_id::DOUBLE) {
            throw std::runtime_error("you can't || or && with double");
          }
          switch (rhs.type_) {
            case type_id::INTEGER:
            case type_id::BOOL:
              return ctx.analyze_type_table_["bool"];
            default:
              throw std::runtime_error("you can't || or && with other types than int or bool");
          }
      }
      break;
    case type_id::STRING: {
    ...
    case BOOL: {
      switch (op) {
        ...
        default:
          throw std::runtime_error("only || and && operations are allowed for bool");
      }
    }
